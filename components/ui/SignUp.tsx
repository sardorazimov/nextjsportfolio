...
export const WavyDotPattern = ({
      ...
      dots.push(
        ...
        >
          <div
            className={cn(
              "dot-point",
              "rounded-xl bg-gradient-to-r from-[#9946dc] bg-purple-400 opacity-35 hover:from-black hover:bg-purple-700 hover:opacity-100 animate-ping shadow shadow-purple-900",
              DOT_WIDTH,
              DOT_HEIGHT,
            )}
            data-index={index}
  ...
  return (
    <div
      ...
      className={cn("w-full h-full absolute z-0 grid", className)}
    >
      {dots.map((dot) => dot)}
    </div>
  );
